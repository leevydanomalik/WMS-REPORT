...
public class SAPActivityPostingRESTController {
    
    private final String SAP_ACTIVITY_POSTING_CSV = "sap_activity_posting.jasper";
     
    @Autowired 
    ReportService reportService;
    
    @ResponseBody
    @RequestMapping(value = "data.sap.activity.posting.csv/", method = RequestMethod.GET,
            produces = "text/csv")
    public byte[] generateSAPActivityPostingReportCSV(@RequestParam(value = "reportFormat", required = false) FileExtention reportFormat ) throws GenericException {
        ...
        os = (ByteArrayOutputStream) reportService.showReportJdbcDataSourceExportToPdfTxtCsvXls(FileExtention.CSV, SAP_ACTIVITY_POSTING_CSV,params);
