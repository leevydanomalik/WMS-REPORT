...
public class PricingRESTController {
    ...
    private final String PRICING_NEW_CSV = "pricing_new.jasper";
    ...
    @RequestMapping(value = "/data.pricing/{clientID}/{companyID}",
            method = RequestMethod.GET,
            produces = "text/csv")
    public byte[] generatePricingReportByES(@PathVariable("clientID") String clientID, @PathVariable("companyID") String companyID) {
        ...
        reportService.setRptResourcePrefix("/report/");
        try {
            os = (ByteArrayOutputStream) reportService.showReportJdbcDataSourceExportToPdfTxtCsvXls(FileExtention.CSV, PRICING_NEW_CSV, params);
        } catch (GenericException ex) {
            Logger.getLogger(PricingRESTController.class.getName()).log(Level.SEVERE, null, ex);
        }
